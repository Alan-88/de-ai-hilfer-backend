...
def create_message(
    role: Literal["system", "user", "assistant"], **kwargs
    ...
    if role == "system":
        ...
        if "image_data" in kwargs and "mime_type" in kwargs:
            content.append(
                ImageBlock(
                    image_data=kwargs["image_data"], mime_type=kwargs["mime_type"]
                )
            )
        ...
        if "tool_requests" in kwargs:
            content.extend(
                [
                    ToolCallRequestBlock(**tool_request)
                    for tool_request in kwargs["tool_requests"]
                ]
            )
    ...
    if not content:
        print(
            f"警告：调用 create_message 时，为 role='{role}' 提供的参数不足，无法创建消息。"
        )
        return None
