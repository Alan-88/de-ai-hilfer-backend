...
class BaseLLMAdapter(ABC):
    ...
    def __init__(
        self, model: str, api_key_env: str, base_url: str, model_params: ModelParams
    ):
        """
        初始化适配器。
    ...
    @staticmethod
    @abstractmethod
    def unpack_response(response: Any) -> InternalMessageUnion | None:
        """
        [抽象静态方法] 将特定于 API 的响应解包成内部 InternalMessageUnion 格式。
    ...
    @abstractmethod
    def chat(
        self, messages: ConversationHistory, tool_tags: str | List[str] = [], **kwargs
    ) -> InternalMessageUnion | None:
        """
        与大语言模型进行对话。
    ...
    def check_api_key(self):
        if self.api_key is None:
            print(
                f"警告：环境变量 '{self.api_key_env}' 未设置，模型 '{self.model}' 可能无法使用。"
            )
...
class GeminiAdapter(BaseLLMAdapter):
                ...
                if isinstance(block, TextBlock):
                    ...
                    parts.append(
                        types.Part.from_bytes(
                            data=block.image_data, mime_type=block.mime_type
                        )
                    )
                    ...
                    parts.append(
                        types.Part.from_function_call(name=block.tool_name, args=args)
                    )
                elif isinstance(block, ToolResultBlock):
                    # 将我们的 Block 转换为 Gemini 的 FunctionResponse
                    parts.append(
                        types.Part.from_function_response(
                            name=block.tool_name, response={"content": block.result}
                        )
                    )
        ...
        try:
            if not response.candidates:
                ...
                return create_message(
                    role="assistant", text="模型没有返回任何有效内容。"
                )
                    ...
                    tool_requests.append(
                        ToolCallRequestBlock(
                            ...
                            arguments_json=json.dumps(
                                {k: v for k, v in fc.args.items()}, ensure_ascii=False
                            ),
                        )
                    )
                # **新的健壮文本提取逻辑**
    ...
    def chat(
        ...
        try:
            ...
            print(
                f"错误：调用 Gemini API 时出错 ({self.model}: {model_selected}) -> {e}"
            )
            raise e
...
class OpenAIAdapter(BaseLLMAdapter):
        ...
        for msg in history:
            if isinstance(msg, SystemInternalMessage):
                openai_messages.append(
                    {"role": "system", "content": msg.content[0].text}
                )
                ...
                final_content = (
                    content_parts
                    if has_image
                    else "".join(p["text"] for p in content_parts)
                )
                ...
                tool_requests = [
                    block
                    for block in msg.content
                    if isinstance(block, ToolCallRequestBlock)
                ]
    ...
    def unpack_response(
        ...
        """
        [翻译机] 将 OpenAI API 的响应解包成内部 InternalMessageUnion 格式。
        """
    ...
    def chat(
        ...
        try:
            ...
            print(
                f"错误：调用 OpenAI API 时出错 ({self.model}: {model_selected}) -> {e}"
            )
            raise e
...
class OllamaAdapter(BaseLLMAdapter):
    ...
    @staticmethod
    def unpack_response(response: dict) -> InternalMessageUnion | None:
        """
        [翻译机] 将 Ollama API 返回的字典响应解包成内部 InternalMessageUnion 格式。
        """
    ...
    def chat(
        ...
        try:
            ...
            print(
                f"错误：调用 Ollama API 时出错 ({self.model}: {model_selected}) -> {e}"
            )
            raise e
