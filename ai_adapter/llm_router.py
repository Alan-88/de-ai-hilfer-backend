...
class ChatSession:
    ...
    def __init__(
        self, adapters: List[BaseLLMAdapter], system_prompt: str, router: "LLMRouter"
    ):
    ...
    async def run(
        ...
        message: str,
        max_turns: int = 5,
        enabled_tags: List[str] | str = None,
        **kwargs,
        ...
        async def reason() -> Union[InternalMessageUnion, None]:
        ...
        async def execute_tool_calls(
            tool_requests: List[ToolCallRequestBlock],
            ...
            for request_block in tool_requests:
                ...
                if tool_function:
                    ...
                    tool_result = (
                        f"错误：Agent 尝试调用一个不存在的工具 '{tool_name}'。"
                    )
        ...
        for i in range(max_turns):
            response: InternalMessageUnion = await reason()
            if response is None:
                print("错误：所有模型均调用失败。")
                break
            ...
            if isinstance(response, AssistantInternalMessage):
                ...
                if tool_requests:
                    tool_results: List[ToolResultBlock] = await execute_tool_calls(
                        tool_requests
                    )
...
class LLMRouter:
        ...
        for model, config in self.config.models.items():
            ...
            if config.provider in self.adapter_map:
                ...
                try:
                    ...
                    result_models[model] = AdapterClass(
                        **{k: v for k, v in adapter_args.items() if v is not None}
                    )
                    print(
                        f"信息：成功初始化模型 '{model}' (Provider: {config.provider})。"
                    )
                except Exception as e:
    ...
    def _get_sorted_adapters(self) -> List[BaseLLMAdapter]:
        ...
        enabled_models = [
            (n, c)
            for n, c in self.config.models.items()
            if c.enabled and n in self.models
        ]
        sorted_models = sorted(enabled_models, key=lambda item: item[1].priority)
        return [self.models[name] for name, _ in sorted_models]

    def get_session(
        self, session_id: str, system_prompt_override: str = None
    ) -> ChatSession:
        """
        获取或创建一个会话。
    ...
    async def chat_with_failover(
        adapters: List[BaseLLMAdapter], messages: ConversationHistory, **kwargs
    ) -> Optional[InternalMessageUnion]:
        if not adapters:
            return None
        for adapter in adapters:
            try:
                ...
                print(
                    f"!!! [Failover] 模型 '{adapter.model}' 调用失败: {e}。尝试下一个..."
                )
                continue
